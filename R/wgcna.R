...
#'
#' @param result DESeq2 results object
...
WgcnaClass <- R6::R6Class("WgcnaClass",
  ...
  public = list(
    ...
    initialize = function(result, datTraits, degenes_only=FALSE, nGenes=5000, genecode=FALSE)
    {
      ...
      if(genecode){
        ensembl_ids <- genecode2ensembl(colnames(WGCNA_matrix))
        colnames(WGCNA_matrix) <- ensembl_ids
      }else{
    ...
    selectHubs = function()
    {
      hub <- WGCNA::chooseTopHubInEachModule(self$matrix, self$moduleColors,
                                             power = self$sft$powerEstimate)
      hub <- data.frame(ensembl_id=hub)
      annot <- hlsgr::txdb$gene[,c('ensembl_id', 'gene', 'description')]
      hubgenes <- merge(hub, annot, by="ensembl_id", sort=FALSE)
    ...
    {
      annot <- hlsgr::txdb$gene[,c("ensembl_id", "entrez_id", "gene")]
    ...
    exportCytoscape = function(threshold = 0.02)
    {
      ...
      annot = hlsgr::txdb$gene[,c("ensembl_id","gene")]
      ...
      for (module in unique_colors)
      {
        ...
        modGenes = annot$gene[match(modProbes, annot$ensembl_id)]
    ...
    calcPvalue = function()
    {
      ...
      casecontrol = as.data.frame(self$datTraits$group)
      names(casecontrol) = "casecontrol"
      # names (colors) of the modules
      modNames = substring(names(self$net$MEs), 3)
      geneModuleMembership = as.data.frame(cor(self$matrix, self$net$MEs, use = "p"))
      MMPvalue = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership), ncol(self$matrix)))

      names(geneModuleMembership) = paste("MM", modNames, sep="")
      names(MMPvalue) = paste("p.MM", modNames, sep="")
      geneTraitSignificance = as.data.frame(cor(self$matrix, casecontrol, use = "p"))
      GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), ncol(self$matrix)))
      names(geneTraitSignificance) = paste("GS.", names(casecontrol), sep="")
      names(GSPvalue) = paste("p.GS.", names(casecontrol), sep="")

      GSPvalue_df = data.frame(ensembl_id=rownames(GSPvalue), pvalue=GSPvalue[1])
      annot = hlsgr::txdb$gene[,c("ensembl_id","gene")]
      GSPvalue_df_annot = merge(GSPvalue_df, annot, by="ensembl_id", sort=FALSE)
      write.csv(GSPvalue_df_annot, paste0(outdir, "/gspvalue.csv"))

      self$GSPvalue <- GSPvalue_df_annot
      invisible(self)
    }
